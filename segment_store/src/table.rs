...
impl Table {
    ...
    pub fn aggregate<'input>(
        ...
        if !self.has_all_columns(&group_columns) {
            todo!() //TODO(edd): return an error here "group key column x not found"
        }

        if !self.has_all_columns(&aggregates.iter().map(|(name, _)| *name).collect::<Vec<_>>()) {
            todo!() //TODO(edd): return an error here "aggregate column x not found"
        }

        if !self.has_all_columns(&predicates.iter().map(|(name, _)| *name).collect::<Vec<_>>()) {
            todo!() //TODO(edd): return an error here "predicate column x not found"
        }
